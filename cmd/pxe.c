	...
	if (bootm_argv[3]) {
		...
		if (fdtfile) {
			...
			if (!strcmp(env_get("lcd_exist"), "0")) {
				...
				// Set fbdev size to AML101 MIPI LCD resolution 1200x1920
				run_command("fdt addr ${fdt_addr_r}; fdt resize 65536; fdt set /drm-subsystem fbdev_sizes <1200 1920 1200 3840 32>;", 0);
			}
		} else {
			bootm_argv[3] = NULL;
		}
